...
const EmployeesCrudForm = ({ item, itemType, isCreate, handleObjectType }) => {
    /**itemTypes:
     * 1: Books
     * 2: Members
     * 3: Employees
    ...
    const [result, setResult, isLoading] = (!isCreate ? hooks.useGetDataById('employees', item) : '');
    ...
    const sendData = async (props) => {
        console.log("pasa x el update antes????");
        let result;
        if (!isCreate) {
            console.log("pasa x el update????");
            result = await hooks.useCreateOrUpdate('employees', isCreate, item, props);
            if (result) {
                setAlertVariant('success');
                setAlertText("El empleado fue actualizado correctamente.");
            } else {
                setAlertVariant('danger');
                setAlertText("Ocurrió un error al actualizar el empleado.");
            }
        } else {
            result = await hooks.useCreateOrUpdate('employees', isCreate, null, props);
            if (result) {
                setAlertVariant('success');
                setAlertText("El empleado fue creado correctamente.");
            } else {
                setAlertVariant('danger');
                setAlertText("Ocurrió un error al crear el empleado.");
            }
        }
        hooks.useClearFields('employees', false);
        setShow(true);
        setTimeout(() => {
            setShow(false);
        }, 5000);
    ...
    if (isLoading) {
        ...
        console.log("cómo está el iscreate", isCreate, data);
        ...
        content = (
            <>
                <Formik
                    ...
                    console.log("pasó x el change", data);
                    }
                    }
                    ...
                    {({ errors,
                        ...
                        <Form onSubmit={handleSubmit}>
                            ...
                            <div className="btns-container">
                                ...
                                <Button variant="danger" type="button" onClick={() => { setAction(1); handleObjectType(1, 3, 'Empleados', 'employees'); }}>
                                    Cancel
                                </Button>
                            </div>
                        </Form>
